...
struct Leaf {
	int req_payload;	//payload 
	int res_payload;		//payload 
	u64 timestamp;
	u64 req_timestamp;
	u64 res_timestamp;
	int http_code;
...
int http_filter(struct __sk_buff *skb) {
	...
	http_code = http_code_atoi(p[9], p[10], p[11]);
	if ((http_code == 200) || (http_code == 401)) {
	    key.dst_ip = ip->src;
 	    key.src_ip = ip->dst;
	    key.dst_port = tcp->src_port;
	    key.src_port = tcp->dst_port;
	    struct Leaf *look_leaf = sessions.lookup(&key);
	    if (look_leaf) {
	        zero = *look_leaf;
			zero.timestamp = bpf_ktime_get_ns() - zero.req_timestamp;
	        zero.res_payload = payload_length;
	        zero.res_timestamp = bpf_ktime_get_ns();
	        zero.http_code = http_code;
		sessions.update(&key, &zero);
	    }
	} else {
	    zero.req_payload = payload_length;
	    zero.req_timestamp = bpf_ktime_get_ns();
	    zero.http_code = http_code;
	    sessions.lookup_or_init(&key,&zero);
	}
	goto DROP;
